...
public class ManagedAsyncMQTTClient {
	private static MqttClient client;
	private static ManagedAsyncMQTTClient mqttManagedClient;
	private static MqttConnectOptions conOpts;
	private static Object lock = new Object();
	private ManagedAsyncMQTTClient() {
		
	}
	
	public static ManagedAsyncMQTTClient getMQTTClient(String brokerUrl, String clientId, MqttConnectOptions connectionOpts) throws MqttException, InterruptedException {
		...
		if(mqttManagedClient == null && null == client) {
			synchronized (lock) {
				if(mqttManagedClient == null && null == client ) {
						...
						mqttManagedClient = new ManagedAsyncMQTTClient();
					}
				}
			}
		reconnect(client);
